import { addDoc, collection } from 'firebase/firestore';
import { userInfo } from 'os';
import React, { ChangeEvent, SetStateAction, useState } from 'react';
import { InputFiles } from 'typescript';
import { db } from '../../../config/firebase-config';
import { LoadingProductPageView } from '../../views/LoadingProductPage/LoadingProductPage';

export const LoadingProductPageContainer = () => {
  const [image, setImage] = useState<any>(
    'data:image/jpeg;base64,',
  );
  const [name, setName] = useState<string>('');
  const [description, setDescription] = useState<string>('');
  const [,] = useState<string>('');

  const handleOnChange = (event: any) => {
    let file = event.target.files[0];
    let reader = new FileReader();
    reader.onloadend = () => {
      console.log(reader.result);
      setImage(reader.result);
    };
    reader.readAsDataURL(file);
  };

  const handleOnChangeSetName = (event: ChangeEvent<HTMLInputElement>) => {
    setName(event.target.value);
  };

  const handleOnChangeSetDescription = (event: ChangeEvent<HTMLTextAreaElement>) => {
    setDescription(event.target.value);
  };

  const handleOnClickLoadProduct = async () => {
    const docRef = await addDoc(collection(db, 'products'), {
      name: name,
      description: description,
      img: image,
      seller: '',
    });
  };

  return (
    <div>
      <LoadingProductPageView handleOnChange={handleOnChange} image={image} handleOnClickLoadProduct={handleOnClickLoadProduct} setName={handleOnChangeSetName} setDescription={handleOnChangeSetDescription} />
    </div>
  );
};
